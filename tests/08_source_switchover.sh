...
setup t08_switchover
trap 'restore_pair $PUB $PUB_STANDBY sub_t08' EXIT
...
sql $SUB $DB "CREATE SUBSCRIPTION sub_t08 CONNECTION '$(pub_conninfo $DB)' PUBLICATION pub_ha"
wait_sync $SUB $DB sub_t08
...
wait_value $PUB $DB "SELECT pg_wal_lsn_diff(pg_current_wal_lsn(), confirmed_flush_lsn) = 0
                       FROM pg_replication_slots WHERE slot_name = 'sub_t08'" t \
...
sql $SUB $DB "ALTER SUBSCRIPTION sub_t08 CONNECTION '$(conninfo $PUB_STANDBY $DB)'"
sql $SUB $DB "ALTER SUBSCRIPTION sub_t08 ENABLE"
